...
*   Fix deletion of empty directories in ActiveSupport::Cache::FileStore.
    修复 ActiveSupport::Cache::FileStore 中删除空目录.
...
*   Prevent `DateTime#change` from truncating the second fraction, when seconds
    ...
    当第二部分不需要修改, 阻止 `DateTime#change` 从第二部分分割分数.

    *Chris Baynes*

*   Added `ActiveSupport::TimeWithZone#to_r` for `Time#at` compatibility.
    添加 `ActiveSupport::TimeWithZone#to_r` 为了兼容 `Time#at`.
...
*   `ActiveSupport::NumberHelper#number_to_human` returns the number unaltered when
    ...
    Fixes #9269.
*   当给定的单位不包含需要的 key 的时候, `ActiveSupport::NumberHelper#number_to_human`
    返回的数字不变, 例如, 当提供的数字小于提供的 key.
    Fixes #9269.
...
*   Extract `ActiveSupport::Testing::Performance` into https://github.com/rails/rails-perftest
    ...
    提取 `ActiveSupport::Testing::Performance` 到 https://github.com/rails/rails-perftest
    你可以添加这个 gem 到你的 `Gemfile`继续使用性能测试.
...
*   **`Hash#extract!` returns only those keys that present in the receiver**.
        ...
        a.extract!(:a)    #=> {:a=>2} 
        a                 #=> {:v=>3} 
        {a: 2, v: 3}.extract(:a) # NoMethodError: undefined method `extract' for {:v=>3}:Hash
