...
*   Skip valid encoding checks for non-String parameters that come
    ...
    Fixes #9435.
*   跳过对来自默认匹配路由的 non-String 参数编码验证.
    Fixes #9435
...
*   Don't verify Regexp requirements for non-Regexp `:constraints`.
    Fixes #9432.
*   对应非正则 `:constraints` 不验证是否需要进行正则验证.
...
*   移除对请求的 XML 参数的解析. 如果仍然解析 XML 参数请安装 `actionpack-xml_parser' gem.
...
*   Add a message when you have no routes defined to both `rake routes` and
    ...
    添加一条消息当你没有定义路由的时候执行 `rake routes` 和 GET "/rails/info/routes" 让你知道你没有定义路由并且链接到 Rails guide.

    *Steve Klabnik*

*   Change `image_alt` method to replace underscores/hyphens to spaces in filenames.
*   更改 `image_alt` 方法替换文件名中的 下划线/连字符 为空格.
...
*   We don't support Ruby constant notation in the `:controller` option for route
    ...
    我们在路由定义中的 `:controller` 选项并不支持 Ruby 常数符号. 因此, 现在会抛出一个 `ArgumentError`:
...
*   `assert_template` can be used to verify the locals of partials,
    ...
    `assert_template` 可以用来验证当前目录中的局部模板.
...
*   Fix `content_tag_for` with array HTML option.
    ...
    修复有 array HTML 选项的 `content_tag_for`. 它会将输入作为字符串嵌入而不是直接插入.
...
*   Change the behavior of route defaults so that explicit defaults are no longer
    ...
    改变路由的 `defaults` 习惯以致于明确的 `defaults` 不再需要, 这里的键不再是路由的部分. 例如:
    ...
    例如一个功能性测试或者直接使用 `url_for` 将会需要从一个 hash 构造一个 url. 然而明确的使用显式形式改变了这一行为因此这里不再需要:
...
*   Add support for routing constraints other than Regexp and String.
    ...
    添加路由 `constraints` 对正则表达式和字符串之外的支持. 例如这里可以允许像这样使用数组:
    ...
    或者请求方法需要返回 Fixnum 的 `constraints`:
    ...
    注意这里仅仅应用 `constraints` 到请求的路径, `constraints` 仍然需要被指定
    为正则表达式被编译成一个单一的正则表达式。
...
*   Extract `ActionDispatch::PerformanceTest` into https://github.com/rails/rails-perftest
    ...
    提取 `ActionDispatch::PerformanceTest` 到 https://github.com/rails/rails-perftest 你可以添加这个 gem 到你的 Gemfile 继续使用性能测试.
...
*   Do not append second slash to `root_url` when using `trailing_slash: true` (trailing尾随)
    Fix #8700
...
*   Added `Mime::NullType` class. This  allows to use `html?`, `xml?`, `json?`, etc.
    ...
    添加 `Mime::NullType` 类. 这样允许使用 `html?`, `xml?`, `json?` 等等. 当请求的格式未知的时候, 不会抛出异常.
...
*   Integrate the Journey gem into Action Dispatch so that the global namespace
    ...
    集成 Journey gem 到 Action Dispatch 使得这个全局名称空间没有被 model 中的名称污染.
...
*   **Rename all action callbacks from *_filter to *_action to avoid the misconception that these**
    ...
    **重命名 *_filter 为 *_action 避免误解, 这些 callback 仅仅用来改造或者中止响应.**
...
*   Add `cache_if` and `cache_unless` for conditional fragment caching:
    添加 `cache_if` 和 `cache_unless` 用于条件缓存片段:
...
*   Sweepers was extracted from Action Controller as `rails-observers` gem.
    Sweepers 被从 Action Controller 提取到 `rails-observers` gem.
...
*   Allow setting a symbol as path in scope on routes. This is now allowed:
    允许在路由中设置一个 symbol 做为一个路径(的一部分). 现在允许这样:
    ...
    这里也可以传递多个 symbol 到 scope 到快捷的嵌套 scope:
...
*   `assert_template` is no more passing with what ever string that matches
    ...
    *[Hugolnx](https://github.com/HugoLnx/rails/commit/19dff78d0202303f7559921f24b9801722695f33)*

*   `image_tag` will set the same width and height for image if numerical value
    passed to `size` option.
    如果设置 `size` 值为数字则 `image_tag` 将会设定相同的宽度和高度.
...
*   Add `.ruby` template handler, this handler simply allows arbitrary Ruby code as a template. *Guillermo Iguaran*
    添加 `.ruby` template 处理器, 这个处理器能够很容易允许任意 Ruby 代码作为 template. *Guillermo Iguaran*
