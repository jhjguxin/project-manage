...
[Ruby on Rails](http://rubyonrails.com/) is an  **优雅**, **简洁** and **有趣** way to build web applications. Unfortunately, many gotchas 陷阱 await the new programmer. Now that I have a few [rails projects}(http://instacalc.com/) under my belt, here's my shot at sparing you the suffering I experienced when first getting started.
...
"Ruby on Rails" 虽然读起来朗朗上口但是很容易引起混淆. Is Rails some type of magical drug that Ruby is on? (Depending on who you ask, yes.)
...
Ruby scraps 废弃 the annoying 讨厌的, ubiquitous 无处不在的 punctuation 标点符号 that distracts 分散 from the program logic. Why put parens ((around),(everything))? Again, if you want parens, put ‘em in there. But you'll take off the training wheels soon enough.
...
Rails has its own peculiarities 特殊性. "Trust us, it's good for you." say the programmers. It's true – the features/quirks make Rails stand out, but they're confusing until they click. Remember:

  * **Class and table names are important**. Rails has certain naming conventions 约定; it expects objects from the class Person to be saved to a database table named people. Yes, Rails has **a pluralization 多元化 engine** to figure out what object maps to what table (I kid you not). This magic is great, but scary 可怕 at first when you're not sure how classes and tables are getting linked together.
  * Many methods take an "options" hash as a parameter, rather than having dozens 数十 of individual 个别 parameters. When you see
...
There are only two parameters: the name ("View Post") and a hash with 3 key/value pairs. Ruby lets us remove the extra parens and braces, leaving the stripped-down 剥离 function call above.

### Understanding The Model-View-Controller Pattern 模式
...
The MVC pattern is key to building a readable, maintainable 可维护的 and easily-updateable web app.
...
The naming conventions are important – it lets rails applications "find their parts" easily, without additional configuration. Also, it's very easy for another programmer to understand and learn from any rails app. I can take a look at Typo, the rails blogging software, and have a good idea of how it works in minutes. Consistency 一致性 creates comprehension.
